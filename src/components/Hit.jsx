...
function Hit({ hit, highlight: Highlight }) {
  ...
  const handleAddToCart = () => {
    ...
    const itemInCart = currentCart.find((item) => item.objectID === hit.objectID)
  ...
  return (
    <article className="flex flex-col items-center p-2 h-full">
      <div className="flex flex-col items-center flex-grow">
        <Link to={`/product/${encodeURIComponent(hit.objectID)}`} className="flex justify-center w-full">
          {/* ADJUST ATTRIBUTE NAME BELOW */}
          <img src={hit['large image url']} alt={hit['product name']} className="w-1/2 max-w-xs h-auto mb-2 rounded object-cover" />
          {/*  */}
        </Link>
        {/* ADJUST ATTRIBUTE NAME BELOW */}
        ...
        <Highlight attribute="product name" hit={hit} className="text-xl h-12 text-center font-bold mb-2 break-words" />
        {/*  */}
      </div>
      <div className="flex flex-col items-center w-full  mt-4 md:mt-6 lg:mt-8">
        ...
        <div className="flex justify-center items-center space-x-2 mt-2">
          <button className="bg-green-500 text-white text-sm px-2 py-1 rounded" onClick={handleAddToCart}>
            Add to Cart
          </button>
          {notification && (
          ...
          <Link to={`/product/${encodeURIComponent(hit.objectID)}`}>
            <button className="bg-gray-500 text-white text-sm px-2 py-1 rounded">View Details</button>
          </Link>
        </div>
      </div>
    </article>
